...
<div><h3 class="text-center mb-3"> All Sub Category</h3></div>

<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="text-right mb-4">
                <button class="btn btn-primary" data-toggle="modal" data-target="#addsubCategory">Add new +</button>
            </div>
                    ...
                    <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-transparent">
                                ...
                                <td class="text-left">{{ucfirst($subCategory->name)}}</td>
                                <td class="text-left">{{ucfirst($subCategory->status)}}</td>
                                <td>
                                    <button class="btn btn-info" data-toggle="modal" data-toggle="modal" data-target=".updatesubCategory" onclick="editResource('{{ route('sub-categories.edit', $subCategory->id) }}','.updateModalsubCategory')">Update</button>
                                    <button class="btn btn-danger" onclick="commonFunction(true,'{{ route('sub-categories.destroy',$subCategory->id) }}','{{route('sub-categories.index')}}','delete','Are you sure you want to delete?','');">Delete</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal-content">
            <form class="form-valide" id="create-category">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" onclick="commonFunction(false,'{{ route('sub-categories.store') }}','{{route('sub-categories.index')}}','post','','create-category');">Save</button>
                </div>
            </form>
        </div>
    </div>
